...
P=chol(diag([0.5 0.18])/1e3);
Q=chol(diag([1e-5,1e-5]));
R=chol((1e-2)^2);
...
for k=2:length(t)
    ...
    % Q-adaptive
    %% Q learning
    %Q=Q-ffactor*(y(k)-y_pred(k))*(y(k)-y_pred(k))'/Sy;    
end
...
aux=[x(2)+w*randn;
    -(k/m)*x(1)^3-(b/m)*x(2)+F+w*randn];
end

function Y=F(X,u,m,b,kr,Ts,L)
Y=zeros([L,2*L+1]);
    for k=1:2*L+1
        Y(1,k)=X(1,k)+Ts*X(2,k);
        Y(2,k)=X(2,k)+(Ts/m)*(-kr*X(1,k)^3-b*X(2,k)+u);
    end
end
